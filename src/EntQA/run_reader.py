...
def main(args):
    ...
    tokenizer = ElectraTokenizer.from_pretrained(
            'ahotrod/electra_large_discriminator_squad2_512')
    model = load_model(False, args.init_model, args.type_encoder,
    ...
    #         else:
    #             loss_avg.backward()
    #         tr_loss += loss_avg.item() if not torch.isnan(loss_avg).item() else 0
    ...
    #         pbar.set_description(f"loss: {loss_avg.item()}|")
    #         pbar.update(1)
